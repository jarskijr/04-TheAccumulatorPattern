...
#            for _ in range(n):
#        What does the   _   (underscore) mean?             we ignore the loop variable for now.
#
...
#        It updates x by diameter AND by a l of k (only need one or the other, not both)
#
#              x = starting_point.x
#              for k in range(n):
